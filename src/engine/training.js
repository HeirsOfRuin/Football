...
function pickGrowthAttr(rng, player, club) {
...
/** Weekly training tick for one player. */
export function trainPlayer(rng, world, club, player) {
  const rate = developmentRate(world, club, player);
  player.devBank = (player.devBank || 0) + rate + rng.range(-0.02, 0.02);
  let changed = false;
  while (player.devBank >= 1) {
    const attr = pickGrowthAttr(rng, player, club);
...
export function dailyPlayerTick(rng, world, club, player, playedYesterday) {
  ...
  const recovery = playedYesterday ? 3.2 : 6.5;
